...
fun formatDateTime(formatFrom: String, formatTo: String, value: String): String {
    val parser = SimpleDateFormat("$formatFrom")
    val formatter = SimpleDateFormat("$formatTo", Locale.ENGLISH)
    return formatter.format(parser.parse("$value"))
}

fun Context.roundOffDecimal(number: Double): Double? {
    val df = DecimalFormat("#.##", DecimalFormatSymbols(Locale.ENGLISH))
    df.roundingMode = RoundingMode.CEILING // FLOOR for lower value
...
fun getBeforeDate(
    ...
    val myFormat = "dd-MMM-yyyy"  // you can use your own date format
...
fun setAnimFallDown(context: Context): LayoutAnimationController? {
    return AnimationUtils.loadLayoutAnimation(
        context, R.anim.ll_fall_down
    )
}

fun setAnimSlideInFromRight(context: Context): LayoutAnimationController? {
    return AnimationUtils.loadLayoutAnimation(
        context, R.anim.ll_slide_in_from_right
    )
}

fun roundOffDecimal(number: Double): Double {
    return number.toBigDecimal().setScale(0, RoundingMode.HALF_DOWN).toDouble()
}
